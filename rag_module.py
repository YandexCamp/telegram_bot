    ...
    def load_and_split_documents(self, files: List[str]) -> List[Document]:
        ...
        for file_path in files:
            try:
                ...
                # Добавляем метаданные к документам
...
def initialize_rag() -> bool:
    """Функция для инициализации RAG системы (вызывать при запуске бота)"""
    return rag_system.initialize_rag_system()


def rag_pipeline(user_query: str, top_k: int = TOP_K_RESULTS) -> str:
    """Основная функция RAG пайплайна"""
    return rag_system.search(user_query, top_k)


# Функция для обновления векторного хранилища
def update_vectorstore() -> bool:
    ...
    files = rag_system.download_docs_from_s3()
    if not files:
        return False

    chunks = rag_system.load_and_split_documents(files)
    if not chunks:
        return False

    return rag_system.build_vectorstore(chunks)
