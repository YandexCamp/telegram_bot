...
TELEGRAM_SERVICE_URL = os.getenv("TELEGRAM_SERVICE_URL", "http://localhost:9999/api/telegram_bot")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Получено сообщение от {user_id} (@{username}): {message_text}")
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update and update.effective_message:
        await update.effective_message.reply_text("❌ Произошла ошибка при обработке сообщения")
