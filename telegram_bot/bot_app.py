...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_message or not user_message.strip():
        await update.message.reply_markdown_v2(escape_markdown("Пожалуйста, введите вопрос", version=2))
        return
    ...
    await update.message.reply_markdown_v2(escape_markdown(response_text, version=2))


async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.error(f"Update {update} caused error {context.error}")
    if update and update.effective_message:
        await update.effective_message.reply_markdown_v2(escape_markdown("Произошла ошибка. Пожалуйста, попробуйте позже.", version=2))
...
async def clear_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_markdown_v2(escape_markdown("🧹 История диалога очищена. Начните новый диалог.", version=2))
...
async def rag_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id not in admin_ids:
        await update.message.reply_markdown_v2(escape_markdown("❌ У вас нет прав для выполнения этой команды.", version=2))
        return
    await update.message.reply_markdown_v2(escape_markdown("🔄 Начинаю обновление базы документов...", version=2))
    try:
        await context.bot.send_chat_action(chat_id=update.effective_chat.id, action="typing")
        success = update_vectorstore()
        if success:
            yandex_bot.rag_enabled = True
            await update.message.reply_markdown_v2(escape_markdown("✅ База документов успешно обновлена!\n🔍 RAG система активирована.", version=2))
        else:
            await update.message.reply_markdown_v2(escape_markdown("❌ Ошибка при обновлении базы документов.\nПроверьте логи для подробной информации.", version=2))
    except Exception as e:
        logger.error(f"Ошибка обновления RAG: {e}")
        await update.message.reply_markdown_v2(escape_markdown("❌ Произошла ошибка при обновлении базы документов.", version=2))
