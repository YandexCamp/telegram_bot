...
class YandexGPTBot:
    ...
    def __init__(self, telegram_bot_service_url: str = "http://localhost:9999/api/telegram_bot"):
        """
        Инициализация бота
    ...
    async def process_message(self, chat_id: int, user_id: int, message_text: str, username: Optional[str] = None):
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Ошибка микросервиса: {response.status_code} - {response.text}")
                return {
                    "chat_id": chat_id,
    ...
    async def get_bot_status(self):
        ...
        try:
            response = self.session.get(
                f"{self.telegram_bot_service_url.replace('/api/telegram_bot', '')}/api/telegram_bot/status",
                timeout=10
            )
